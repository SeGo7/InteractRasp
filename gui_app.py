...
except ImportError:  # fall back to standard ttk if ttkbootstrap is unavailable
    tb = None

ttk_mod = tb if tb else ttk

API_URL = 'http://127.0.0.1:5000'

BaseTk = tb.Window if tb else tk.Tk


class ScheduleApp(BaseTk):
    def __init__(self):
        if tb:
        ...
        self.notebook = ttk_mod.Notebook(self)
        self.notebook.pack(fill='both', expand=True)

        self.day_frame = ttk_mod.Frame(self.notebook)
        self.week_frame = ttk_mod.Frame(self.notebook)
        ...
        self.class_combo = ttk_mod.Combobox(self.day_frame, textvariable=self.class_var)
        self.class_combo.pack(pady=10)
        self.class_combo.bind('<<ComboboxSelected>>', self.display_day_schedule)

        self.day_tree = ttk_mod.Treeview(self.day_frame, columns=('lesson', 'subject', 'room', 'teacher'), show='headings')
        ...
        self.week_text = ScrolledText(self.week_frame)
